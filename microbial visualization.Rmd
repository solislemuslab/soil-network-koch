...
```{r glasso}
rho = 0.75
# rho: regularization parameter for lasso 
xx = glasso(x, rho = rho)
#col<- colorRampPalette(c("blue", "white", "red"))(20)
#heatmap(x = xx$w, col = col, symm = TRUE)

#corrplot(xx$wi, is.corr=FALSE, tl.col="black", na.label=" ", type = 'lower', diag = FALSE)

invcov = xx 
P = invcov$wi
colnames(P) = colnames(x); rownames(P) = rownames(x)
# check symmetry
if(!isSymmetric(P)) {
...
colnames(parr.corr) = colnames(P); rownames(parr.corr) = colnames(P)
diag(parr.corr) = 0

# build network graph
graph = graph_from_adjacency_matrix(parr.corr, mode="undirected", weighted=TRUE)
